    ...
    def __init__(self, in_channels, middle_channels, out_channels, downsample=False):
        ...
        if self.downsample:
            self.layer = nn.Sequential(
                ...
                #########################################
                conv1x1(in_channels, middle_channels, 2, 0), # stride 2 => downsampling
                conv3x3(middle_channels, middle_channels, 1, 1), # doesn't change spatial size
                conv1x1(middle_channels, out_channels, 1, 0) # doesn't change spatial size
            )
            self.downsize = conv1x1(in_channels, out_channels, 2, 0) # downsampling the input so we can add it to the output
            ...
            self.layer = nn.Sequential(
                ...
                ############# fill in here (20 points)
                #########################################
                conv1x1(in_channels, middle_channels, 1, 0), # doesn't change spatial size
                conv3x3(middle_channels, middle_channels, 1, 1), # doesn't change spatial size
                conv1x1(middle_channels, out_channels, 1, 0) # doesn't change spatial size
            )
            self.make_equal_channel = conv1x1(in_channels, out_channels, 1, 0)  # doesn't change spatial size
...
class ResNet50_layer4(nn.Module):
    def __init__(self, num_classes= 10 ): # Hint : How many classes in Cifar-10 dataset?
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(3, 64, 7, 2, 5), # out = 18x18x64
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(3, 2, 0) # out = 8x8x64
        )
        self.layer2 = nn.Sequential(
            ResidualBlock(64, 64, 256, False), # out = 8x8x256
            ResidualBlock(256, 64, 256, False), # out = 8x8x256
            ResidualBlock(256, 64, 256, True) # out = 4x4x256
        )
        self.layer3 = nn.Sequential(
            ...
            #########################################
            ResidualBlock(256, 128, 512, False), # out = 4x4x512
            ResidualBlock(512, 128, 512, False), # out = 4x4x512
            ResidualBlock(512, 128, 512, False), # out = 4x4x512
            ResidualBlock(512, 128, 512, True) # out = 2x2x512
        )
        self.layer4 = nn.Sequential(
            ...
            ####### you can refer to the 'layer2' above
            #########################################
            ResidualBlock(512, 256, 1024, False), # out = 2x2x1024
            ResidualBlock(1024, 256, 1024, False), # out = 2x2x1024
            ResidualBlock(1024, 256, 1024, False), # out = 2x2x1024
            ResidualBlock(1024, 256, 1024, False), # out = 2x2x1024
            ResidualBlock(1024, 256, 1024, False), # out = 2x2x1024
            ResidualBlock(1024, 256, 1024, False) # out = 2x2x1024
        ...
        self.avgpool = nn.AvgPool2d(2, None) # out = 1x1x1024
